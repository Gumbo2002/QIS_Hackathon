...
def canMove(rect): #checks if desired spot is empty
    tup = state_index(rect)
    return get_state(tup) == 0
...
def validMoves(piece):
    moves = [] #list of tups that hold the valid moves for a position
    pos = state_index(piece.center)
    
    if(pos[1]==1):
        if(pos[1]+1 < 3 and get_state((pos[0], pos[1]+1, pos[2])) != 0): moves.append((pos[0], pos[1]+1, pos[2]))
        if(pos[1]-1 >= 0 and get_state((pos[0], pos[1]-1, pos[2])) != 0): moves.append((pos[0], pos[1]-1, pos[2]))
        if(pos[2]+1 < 3 and get_state((pos[0]+1, pos[1], pos[2]+1)) != 0): moves.append((pos[0], pos[1], pos[2]+1))
        if(pos[2]-1 >= 0 and get_state((pos[0]-1, pos[1], pos[2]-1)) != 0): moves.append((pos[0], pos[1], pos[2]-1))
    elif(pos[2]==1):
        if(pos[1]+1 < 3 and get_state((pos[0]+1, pos[1]+1, pos[2])) != 0): moves.append((pos[0], pos[1]+1, pos[2]))
        if(pos[1]-1 >= 0 and get_state((pos[0]-1, pos[1]-1, pos[2])) != 0): moves.append((pos[0], pos[1]-1, pos[2]))
        if(pos[2]+1 < 3 and get_state((pos[0], pos[1], pos[2]+1)) != 0): moves.append((pos[0], pos[1], pos[2]+1))
        if(pos[2]-1 >= 0 and get_state((pos[0], pos[1], pos[2]-1)) != 0): moves.append((pos[0], pos[1], pos[2]-1))
    else:
        if(pos[1]+1 < 3 and get_state((pos[0], pos[1]+1, pos[2])) != 0): moves.append((pos[0], pos[1]+1, pos[2]))
        if(pos[1]-1 >= 0 and get_state((pos[0], pos[1]-1, pos[2])) != 0): moves.append((pos[0], pos[1]-1, pos[2]))
        if(pos[2]+1 < 3 and get_state((pos[0], pos[1], pos[2]+1)) != 0): moves.append((pos[0], pos[1], pos[2]+1))
    ...
    rectMoves = [index_position(x) for x in moves]
    #return rectMoves #returns rectangle coords for valid moves
    return moves #returns tuples for state array
